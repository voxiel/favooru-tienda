 ...
 */
class favooru_pedidos extends CI_Controller {
  ...
  function index()
  {
    ...
    {
      redirect('account/sign_in/?continue='.urlencode(base_url().'index.php/account/favooru_pedidos'));
    }
    ...
    $all_accounts = $this->account_model->get_pedidos();
    ...
    {
      $current_user = array();
      $current_user['pedidos_id'] = $acc->pedidos_id;
      $current_user['clientes_nombre'] = $acc->cliente_nombres;
      $current_user['clientes_apellido'] = $acc->cliente_apellidos;
      $current_user['producto_descripcion'] = $acc->producto_descripcion;
      $current_user['producto_precio'] = $acc->producto_precio;
      $current_user['pedidos_codigo'] = $acc->pedidos_codigo;
      $current_user['pedido_fecha'] = $acc->pedido_fecha;
      $current_user['afiliados_nombre'] = $acc->afiliados_nombre;
      $current_user['pedido_estado'] = $acc->pedido_estado;
    ...
    $this->load->view('account/favooru_pedidos', $data);
  }
  ...
  function save($id=null)
  {
    ...
    if( ! $is_new )
    {
      $data['update_product'] = $this->account_model->get_product_by_id($id);
    ...
    if ($this->form_validation->run())
    {
      ...
      else
      {
        ...
        redirect("account/favooru_productos");
      }
    }

    // Load manage users view
    $this->load->view('account/manage_products_save', $data);
  }
